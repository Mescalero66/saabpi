...
class i2cmultiplex:
  ...
  def select_port(self,port):
    '''!
      ...
      @note 
      @ When “multiple multiplexer modules” are used on one I2C bus to connect “sensors with same I2C address”, to avoid conflicts,  
      @ it is necessary to ensure that “the currently used channel” is the only enabled one among “all channels in all multiplexers” 
      @ For instance, when one of the channels in the first multiplexer is enabled, in order to use any channel in the second multiplexer,  
      @ please make sure that the channels in the first multiplexer are all disabled.
    '''    
    if(port > 8):
      return
    data = (1<<port) & 0xFF
